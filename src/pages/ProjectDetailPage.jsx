...
import {
  ArrowLeft, Tag, Cpu, FileCode, List, Package,
  CircuitBoard, AlertCircle, ChevronRight,
} from "lucide-react";
import { Link, useParams, useNavigate } from "react-router-dom";
import { projectCategories } from "../data/projects";
import { getAdminProject } from "../utils/adminProjects";
...
/* ─────────────────────────────────────────────────────────────
   HELPERS
───────────────────────────────────────────────────────────── */
function findStaticProject(projectId) {
...
/* ─────────────────────────────────────────────────────────────
   STATIC CONTENT  (hardcoded projects)
───────────────────────────────────────────────────────────── */
...
const String sheet_url = "https://script.google.com/macros/s/YOUR_SCRIPT_ID/exec?name=";
...
void loop() {
  ...
  digitalWrite(BUZZER, HIGH); digitalWrite(LED, HIGH); delay(200);
  digitalWrite(BUZZER, LOW);  digitalWrite(LED, LOW);  delay(200);
  digitalWrite(BUZZER, HIGH); digitalWrite(LED, HIGH); delay(200);
...
void ReadDataFromBlock(int blockNum, byte readBlockData[]) {
  ...
  if (status != MFRC522::STATUS_OK) {
    Serial.print("Auth failed: "); Serial.println(mfrc522.GetStatusCodeName(status)); return;
  }
  status = mfrc522.MIFARE_Read(blockNum, readBlockData, &bufferLen);
  if (status != MFRC522::STATUS_OK) {
    Serial.print("Read failed: "); Serial.println(mfrc522.GetStatusCodeName(status)); return;
  }
...
const staticProjectContent = {
  "rfid-attendance-google-sheets": {
    ...
    schematicImage: "/rfid-circuit.png",
    schematicNotes: [
    ...
    steps: [
      { step: 1, title: "Set up Google Sheets & Apps Script", detail: "Create a new Google Sheet with columns: Date (A), Time (B), Name (C). Go to Extensions → Apps Script, paste the appscript.gs code, replace 'Sheet ID Here' with your spreadsheet's ID. Deploy as a web app with access set to 'Anyone'. Copy the deployment URL." },
      { step: 2, title: "Register names onto RFID Tags", detail: "Before running the main sketch, use register_card.ino to write each person's name into their RFID tag's Block 2. Change blockData to the desired name, upload to NodeMCU, and scan the tag. Repeat for each person." },
      { step: 3, title: "Wire the circuit", detail: "Connect RC522 (SDA→D4, SCK→D5, MOSI→D7, MISO→D6, RST→D3, 3.3V, GND). Connect I2C LCD (GND, VCC→VIN, SDA→D2, SCL→D1). Connect LED to D0 and Buzzer to D8 with a 1K resistor to GND." },
      { step: 4, title: "Install Required Libraries", detail: "In Arduino IDE, install: MFRC522 (by GithubCommunity), LiquidCrystal_I2C (by Frank de Brabander), and the ESP8266 board package. Set board to 'NodeMCU 1.0 (ESP-12E Module)', baud rate 9600." },
      { step: 5, title: "Configure WiFi & Script URL", detail: "Open rfid_attendance.ino. Replace WIFI_SSID and WIFI_PASSWORD with your credentials. Paste the Google Apps Script deployment URL into sheet_url. If LCD doesn't initialise, try changing I2C address from 0x27 to 0x3F." },
...
export default function ProjectDetailPage() {
  ...
  // Try admin project first, then static
  const adminProject = getAdminProject(projectId);
  const staticResult = !adminProject ? findStaticProject(projectId) : null;

  if (!adminProject && !staticResult) {
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center">
        <div className="text-center">
          <AlertCircle className="w-12 h-12 text-red-400 mx-auto mb-4" />
          <h2 className="text-2xl font-bold mb-2">Project not found</h2>
          <Link to="/projects" className="text-orange-400 hover:text-orange-300">← Back to Projects</Link>
        </div>
      </div>
    );
  }
  ...
  // Unify data shape
  let project, category, subCategory, content;

  if (adminProject) {
    // Build category/subCategory from projectCategories for breadcrumb + back nav
    const cat = projectCategories.find((c) => c.id === adminProject.categoryId);
    const sub = cat?.subCategories.find((s) => s.id === adminProject.subCategoryId);
    project     = adminProject;
    category    = cat    || { id: adminProject.categoryId,    name: adminProject.categoryId    };
    subCategory = sub    || { id: adminProject.subCategoryId, name: adminProject.subCategoryId };
    content     = adminProject;
  } else {
    project     = staticResult.project;
    category    = staticResult.category;
    subCategory = staticResult.subCategory;
    content     = staticProjectContent[projectId] || {};
  }

  const codeFiles    = content.codeFiles || [];
  const activeFile   = codeFiles.find((f) => f.id === (activeCodeFile || codeFiles[0]?.id));
  const wiringRows   = content.schematicNotes || content.wiringConnections || [];
      ...
      <div className="max-w-5xl mx-auto relative">
        ...
        <div className="mb-8 animate-in slide-in-from-bottom duration-700">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${difficultyColor[project.difficulty] || difficultyColor.Beginner}`}>
          ...
          <p className="text-gray-400 text-base sm:text-lg max-w-3xl leading-relaxed">{project.description}</p>
        </div>
        ...
        <div className="animate-in slide-in-from-bottom duration-500 delay-150">
          ...
          {activeTab === "overview" && (
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
                ...
                <div className="mt-4 pt-4 border-t border-slate-800 grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className={`text-sm font-medium ${(difficultyColor[project.difficulty] || "text-gray-400").split(" ")[0]}`}>{project.difficulty}</p>
                  </div>
                </div>
              </div>

              {(content.whatYoullBuild || []).length > 0 && (
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
                  <h3 className="text-lg font-bold text-white mb-3 flex items-center gap-2">
                    <span className="text-2xl">🧩</span> What You'll Build
                  </h3>
                  <ul className="space-y-2 text-sm text-gray-300">
                    {(content.whatYoullBuild || []).map((item, i) => (
              ...
              {(content.howItWorks || []).some((h) => h.label) && (
                <div className="sm:col-span-2 bg-slate-900/50 border border-orange-500/20 rounded-2xl p-6">
                  <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                    <span className="text-2xl">⚡</span> How It Works
                  </h3>
                  <div className="grid sm:grid-cols-4 gap-3">
                    {(content.howItWorks || []).filter((h) => h.label).map((item) => (
                      <div key={item.n} className="bg-white/5 rounded-xl p-4 text-center">
                        <div className="w-8 h-8 rounded-full bg-orange-500/20 text-orange-400 font-bold text-sm flex items-center justify-center mx-auto mb-2">{item.n}</div>
            ...
            <div className="space-y-4">
              {content.schematicImage ? (
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden">
                  ...
                  <div className="p-4">
                    <img src={content.schematicImage} alt="Circuit Diagram" className="w-full rounded-xl object-contain max-h-[480px]" />
                  </div>
                </div>
              ) : (
                <div className="bg-slate-900/50 border border-slate-800 border-dashed rounded-2xl p-10 text-center">
                  ...
                  <p className="text-gray-500 text-sm">No diagram uploaded yet.</p>
                </div>
              )}
          ...
          {activeTab === "steps" && (
            <div className="space-y-4">
              {(content.steps || []).length === 0 ? (
                <div className="text-center py-12 text-gray-500">No steps added yet.</div>
              ) : (
                (content.steps || []).map((item) => (
                  <div key={item.step} className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 sm:p-6 flex gap-4 hover:border-orange-500/20 transition-colors duration-300 group">
          ...
          {activeTab === "code" && (
            <div className="space-y-3">
              {codeFiles.length === 0 ? (
                <div className="text-center py-12 text-gray-500">No code files uploaded yet.</div>
              ) : (
                <>
                    ...
                    {codeFiles.map((f) => (
                      <button key={f.id}
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono whitespace-nowrap transition-all duration-200 border ${
                          (activeCodeFile || codeFiles[0]?.id) === f.id
                            ? "bg-orange-500/20 text-orange-300 border-orange-500/30"
                            : "bg-white/5 text-gray-400 border-white/10 hover:bg-white/10"
                        }`}
                      >
                        <FileCode className="w-3 h-3" />{f.label}
                      </button>
                    ))}
                  </div>
                  {activeFile && (
                    <div className="relative bg-gray-900/50 border border-slate-800 rounded-2xl overflow-hidden">
                      <div className="flex items-center gap-2 px-4 py-3 bg-white/5 border-b border-slate-800">
                        ...
                        <span className="ml-2 text-xs text-gray-400 font-mono">{activeFile.label}</span>
                        <span className="ml-auto text-xs text-gray-600">{activeFile.hint}</span>
                      </div>
                      <SyntaxHighlighter
                        language={activeFile.lang}
                        style={nightOwl}
                        showLineNumbers
                        customStyle={{ margin: 0, background: "transparent", fontSize: "0.78rem", lineHeight: "1.6", padding: "1.25rem" }}
                      >
                        {activeFile.code}
                      </SyntaxHighlighter>
                    </div>
        ...
        <div className="mt-10 pt-6 border-t border-slate-800">
          <button onClick={() => navigate(`/projects?expand=${category.id}`)}
